// @samjonaidi-ship-it/universal-auth | src/extendability/registry.ts | v1.0.0-rc.1 | 2026-04-24 | BB
...
export function registerNotificationChannel(adapter: NotificationChannelAdapter): void {
  if (channels.has(adapter.channel_key)) {
    throw new Error(
      `[@samjonaidi-ship-it/universal-auth] Notification channel '${adapter.channel_key}' is already registered.`
    );
  }
