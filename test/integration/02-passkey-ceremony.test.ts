// @samjonaidi-ship-it/universal-auth | test/integration/02-passkey-ceremony.test.ts | v1.0.0-rc.1 | 2026-04-28 | BB
...
describe('Integration #2 — passkey ceremony (§11.3)', () => {
  it('register options → verify, then authenticate options → verify', async () => {
    ...
    const verifyRes = await bff<{ ok: true; credential: { credential_id: string } }>(
    ...
    const authVerify = await bff<{
      ...
      method: 'POST',
      body: { assertion, device_id: 'test-device-passkey' },
    });
